...
bool test_main(void) {
  ...
  bool observed_timeout_status;
  dif_kmac_error_t err_status;
  kmac_edn_timeout_t timeout_test;
  ...
  dif_kmac_config_t config = (dif_kmac_config_t){
      ...
      .entropy_prescaler = 0x0000,
      .entropy_wait_timer = 0x0001,
  ...
  size_t digest_l =
      kmac_test_vector.digest_len_is_fixed ? kmac_test_vector.digest_len : 0;
  ...
  for (size_t i = 0; i < 100; i++) {
    ...
    CHECK_DIF_OK(dif_kmac_mode_kmac_start(&kmac, &kmac_operation_state,
                                          kmac_test_vector.mode, digest_l,
                                          &kmac_test_vector.key, s_buff));

    CHECK_DIF_OK(dif_kmac_absorb(&kmac, &kmac_operation_state,
                                 kmac_test_vector.message,
    ...
    if (irq_err_pending) {
      ...
      CHECK(err_status == kDifErrorNone ||
                err_status == kDifErrorEntropyWaitTimerExpired,
            "Error other than EDN timeout occurred.");
    }

    observed_timeout_status = irq_err_pending;
    if (observed_timeout_status) {
