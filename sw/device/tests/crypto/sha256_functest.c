...
static status_t long_test(void) {
  otcrypto_const_byte_buf_t msg_buf = {
      .data = (unsigned char *) kLongMessage,
      .len = sizeof(kLongMessage),
  };
  uint32_t act_digest[kHmacDigestNumWords];
  otcrypto_hash_digest_t digest_buf = {
      .data = act_digest,
  ...
  TRY(otcrypto_hash(msg_buf, digest_buf));
  TRY_CHECK_ARRAYS_EQ((unsigned char*) act_digest, kLongMessageDigest, sizeof(kLongMessageDigest));
  return OK_STATUS();
}
