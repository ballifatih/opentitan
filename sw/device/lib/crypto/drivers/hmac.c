...
status_t hmac_init(hmac_ctx_t *ctx, const hmac_mode_t hmac_mode,
  ...
  switch (hmac_mode) {
    ...
    case kHmacModeSha384:
      ...
      // The unit for `ctx->digest_len` is words.
      ctx->digest_len = kHmacSha384DigestWords;
      break;
    ...
    case kHmacModeSha512:
      ...
      // The unit for `ctx->digest_len` is words.
      ctx->digest_len = kHmacSha512DigestWords;
      break;
    default:
      return OTCRYPTO_BAD_ARGS;
  }
...
status_t hmac_oneshot(const hmac_mode_t hmac_mode,
  ...
  hmac_ctx_t hmac_ctx;
  HARDENED_TRY(hmac_init(&hmac_ctx, hmac_mode, key));
  HARDENED_TRY(hmac_update(&hmac_ctx, data, len));
  return hmac_final(&hmac_ctx, digest);
}
